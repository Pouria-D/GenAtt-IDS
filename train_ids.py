...
def parse_arguments(arguments):
    ...
    parser.add('--save_config', required=False, default=None, type=str,
               help='path of config file where arguments can be saved')
    parser.add('--save_model', required=False, default=None, type=str, help='path of file to save trained model')
    parser.add('--algorithm', required=False, choices=['baseline', 'dt', 'knn', 'lr', 'mlp', 'nb', 'rf', 'svm'],
...
def parse_ids_arguments(parser):
    parser.add('--iterations', required=False, type=int, default=1000,
               help='number of training iterations (default 1000)')

    knn_group = parser.add_argument_group('knn')
    knn_group.add('--n_neighbors', required=False, default=5, type=int,
                  help='number of neighbours to compare (default 5)')
    knn_group.add('--knn_algorithm', required=False, default='auto', choices=['auto', 'ball_tree', 'kd_tree', 'brute'],
                  help='algorithm used to compute nearest neighbour (default auto)')
    knn_group.add('--knn_weights', required=False, default='uniform', choices=['uniform', 'distance'],
                  help='importance of neighbour points in classification (default uniform)')

    lr_group = parser.add_argument_group('lr')
    lr_group.add('--lr_solver', required=False, default='lbfgs', choices=['lbfgs', 'sag', 'saga', 'liblinear'],
                 help='algorithm to solve logistic regression problem (default lbfgs)')

    tree_group = parser.add_argument_group('trees (decision tree and random forest)')
    tree_group.add('--n_trees', required=False, default=10, type=int,
                   help='number of trees in a random forest (default 5)')
    tree_group.add('--max_depth', required=False, default=None, type=null_or_int,
                   help='maximum tree depth (default infinite)')
    tree_group.add('--min_samples_leaf', required=False, default=1, type=int,
                   help='minimum number of samples required for a node to be a leaf (default 1)')
    tree_group.add('--min_samples_split', required=False, default=2, type=int,
                   help='minimum number of samples required to split a node (default 2)')
    tree_group.add('--split_criterion', required=False, default='gini', choices=['gini', 'entropy'],
                   help='criterion for how to split a node (default gini)')
    tree_group.add('--splitter', required=False, default='best', choices=['best', 'random'],
                   help='strategy used to split nodes (default best)')
    ...
    mlp_group.add('--log_every', required=False, default=100, type=int,
                  help='log training every this amount of batches (default 100)')
    mlp_group.add('--evaluate_every', required=False, default=1000, type=int,
                  help='log training on validation set every this amount of batches (default 1000)')
    ...
    mlp_group.add('--weight_decay', required=False, default=0, type=float,
                  help='weight decay/L2 regularization strength (default 0)')
